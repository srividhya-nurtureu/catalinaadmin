    ...
    <table class="table table-hover table-striped bg-light" id="plist">
        ...
        while($row = $qry->fetch_assoc()):
            $stock_in = $conn->query("SELECT SUM(quantity) as `total` 
                                      FROM `stock_list` 
                                      WHERE UNIX_TIMESTAMP(`expiry_date`) >= UNIX_TIMESTAMP(CURRENT_TIMESTAMP) 
                                      AND id = '{$row['id']}'")->fetch_assoc();
            $stock_out = $conn->query("SELECT SUM(quantity) as `total` 
                                       FROM `transaction_items` 
                                       WHERE id = '{$row['id']}'")->fetch_assoc();

            $stock_in_total = isset($stock_in['total']) && is_numeric($stock_in['total']) ? $stock_in['total'] : 0;
            $stock_out_total = isset($stock_out['total']) && is_numeric($stock_out['total']) ? $stock_out['total'] : 0;

            $qty = $stock_in_total - $stock_out_total;
            $qty = $qty > 0 ? $qty : 0;
        ?>
        <tr class="item <?php echo $qty < 50 ? 'bg-danger bg-opacity-25' : '' ?>" data-id="<?php echo $row['id'] ?>">
            <td class="td py-0 px-1 pname"><?php echo htmlspecialchars($row['cname']) ?></td>
            <td class="td py-0 px-1 pcode"><?php echo htmlspecialchars($row['product_code']) ?></td>
            <td class="td py-0 px-1 name"><?php echo htmlspecialchars($row['name']) ?></td>
...
<script>
   $(function(){
    ...
    $('#plist tbody tr').click(function(){
        ...
        var cakeprice = _tr.find('.price').text().replace(/,/gi,'');
        ...
        var ntr  = $("<tr tabindex='0'>")
            ...
            .append('<td class="py-0 px-1 align-middle"><div class="fs-6 mb-0 lh-1">'+pcode+'<br/>'+
                    ...
                    '</td>')
            .append('<td class="py-0 px-1 align-middle text-end total">'+parseFloat(cakeprice).toLocaleString('en-US', {style:'decimal', maximumFractionDigits:2})+'</td>');
...
function calculate_total(){
    ...
    $('#total').text(parseFloat(sub).toLocaleString('en-US', {style:'decimal', maximumFractionDigits:2}));
    $('#subTotal').text(parseFloat(sub).toLocaleString('en-US', {style:'decimal', maximumFractionDigits:2}));
    discount = sub * .12;
    $('#tax').text(parseFloat(discount).toLocaleString('en-US', {style:'decimal', maximumFractionDigits:2}));
}
